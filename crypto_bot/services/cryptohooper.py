...
class CryptoHooperHandler(object):
    """
    Another service handler capable of
    handling requests and gathering data
    for the currencies specified from cryptohooper.
    """
    ...
    def token(self, value):
        """
        sets the value passed as a token for the api to use
        :param value:str
        :return:None
        """
        self._token = value
    ...
    def _listing_items(self, url, **kwargs):
        ...
        response = req.get(
            url, headers={"access-token": self.token}, params=data
        )
        if response.status_code != 200:
            raise Exception(
    ...
    def _delete_item(self, url, **kwargs):
        ...
        response = req.delete(
            url, headers={"access-token": self.token}, kwargs=kwargs
        )
        if response.status_code != 200:
            raise Exception(
    ...
    def _create_item(self, url, data):
        ...
        result = {"success": False}
        response = req.post(
            url, data=data, headers={"access-token": self.token}
        )
        if response.status_code != 200:
            raise Exception(
    ...
    def _get_item(self, url, **kwargs):
        ...
        """
        response = req.get(
            url, headers={"access-token": self.token}, params=kwargs
        )
        return response.json()

    def change_token(self):
        """
        refreshes the token required for the requests
    ...
    def get_base_hopper_config(self, hopper_id):
        ...
        :return: json
        """
        url = "{0}hopper/{1}/config".format(
            self.endpoints["api_root"], hopper_id
        )
        return self._get_item(url)
    ...
    def update_hopper_config_pool(
        self, hopper_id, config_pool_id, config_pool_data
    ):
        """
    ...
    def list_templates(self, basic=False) -> json:
        ...
        """
        url = (
            f"{self.endpoints['api_root']}template"
            if not basic
            else f"{self.endpoints['api_root']}template/basic"
        )
        return self._listing_items(url)
    ...
    def list_template_market(self, exchange):
        """
    ...
    def hopper_trade_history_details(self, hopper_id, trade_id):
        ...
        """
        url = (
            f"{self.endpoints['api_root']}hopper/{hopper_id}/trade/{trade_id}"
        )
        return self._get_item(url)
    ...
    def set_hopper_take_profit(
        self, hopper_id, position_id, percentage_profit
    ):
        """
    ...
    def get_hopper_order(self, hopper_id, order_id):
        ...
        """
        url = (
            f"{self.endpoints['api_root']}hooper/{hopper_id}/order/{order_id}"
        )
        return self._get_item(url)

...
class SpreadDetection(SpreadABC):
    def __init__(self, exchange_one: Exchange, exchange_two: Exchange):
        ...
        if exchange_one.currency_pair != exchange_two.currency_pair:
            logger.debug(
                "Spread between different currency pairs is not supported"
            )
            raise SpreadDifferentCurrenciesError(
                "Spread between different currency pairs is not supported"
            )
    ...
    def summary(self):
        return (
            f"{self.exchange_buy} [{self.exchange_buy.last_ask_price}] -> "
    ...
    def _calculate_spread(self):
        ...
        if None in [
            self.exchange_one.last_bid_price,
            self.exchange_one.last_ask_price,
            self.exchange_two.last_bid_price,
            self.exchange_two.last_ask_price,
        ]:
            logger.warning(
                "Cannot calculate this spread because one of the prices is missing."
            )
            raise SpreadMissingPriceError(
                "Cannot calculate this spread because one of the prices is missing."
            )

        d1 = int(
            self.exchange_one.last_bid_price - self.exchange_two.last_ask_price
        )
        d2 = int(
            self.exchange_two.last_bid_price - self.exchange_one.last_ask_price
        )
